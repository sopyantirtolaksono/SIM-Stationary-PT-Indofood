<?php
	 ...
	 // koneksi ke database
      ...
      // pencarian data penggunaan barang
  		$sqlPenggunaan = "SELECT * FROM tbl_riwayat_pesanan WHERE id_barang = '$namaBarang' AND id_departement = '$departement' AND tanggal BETWEEN '$dateRange1' AND '$dateRange2' ORDER BY id_riwayat_pesanan DESC ";
     	...
     	<tbody>
          ...
          <tr>
            <th colspan="8" class="text-center">
              <h6>Total <strong><?=$value["nama_barang"]; ?></strong> yang digunakkan <strong><?=$value["kode_nama"]; ?></strong> dari tanggal <strong><?=$dateRange1; ?> - <?=$dateRange2; ?></strong> adalah <strong><?=$total." ".$value["satuan_barang"]; ?></strong></h6>
            </th>
          </tr>
          <?php } ?>
      	</tfoot>
